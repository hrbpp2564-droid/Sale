...
(function () {
  ...
  function Icon({ name, size = 16, strokeWidth = 1.75, style = {}, ...rest }) {
    const d = P[name];
    if (!d) return React.createElement('span', { style: { display: 'inline-block', width: size, height: size } });
    return React.createElement('svg', {
      width: size, height: size, viewBox: '0 0 24 24', fill: 'none',
      stroke: 'currentColor', strokeWidth, strokeLinecap: 'round', strokeLinejoin: 'round',
      style: { display: 'block', flex: '0 0 auto', ...style }, ...rest,
    }, d.split(' M').map((seg, i) => React.createElement('path', { key: i, d: (i ? 'M' : '') + seg })));
  }
  window.Icon = Icon;
})();
